...
function renderTeamSection(players, gwNumber, isLive, next5GWs, activeGW, sectionType) {
    ...
    players.forEach((pick, idx) => {
        ...
        // Use FPL news for Line 3 and border color
        let newsMessage = '';
        let borderColor = '';
                ...
                ">
                    <div style="display: flex; flex-direction: column; gap: 0.1rem;">
                        ...
                        <div style="font-size: 0.6rem; color: var(--text-secondary); white-space: nowrap;">
                            ${getTeamShortName(player.team)} • ${formatCurrency(player.now_cost)} • ${ownership.toFixed(1)}% • <span style="display: inline-block; padding: 0.2rem 0.4rem; border-radius: 3px; font-weight: 600; font-size: 0.65rem; background: ${formStyle.background}; color: ${formStyle.color};">${formatDecimal(player.form)}</span>
                        </div>
